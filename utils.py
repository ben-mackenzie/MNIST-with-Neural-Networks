...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    #############################################################################
    if shuffle == True:
        #Divakar's answer in https://stackoverflow.com/questions/43229034/randomly-shuffle-data-and-labels-from-different-files-in-the-same-order
        indices = np.arange(len(data))
        random.shuffle(indices)
        indices = np.copy(indices)

        data, label = np.asarray(data), np.asarray(label)
        #data, label = np.array(d[indices].tolist()), np.array(l[indices].tolist()) # got help from the link above to select items in an array by a list of indices
    else:
        indices = np.arange(len(data))
    # generate batches of images w/ required batch size
    ...
    for i, index in enumerate(indices):
        batch.append(index)
        if len(batch) == batch_size or i == l-1:
            batch = np.array(batch)
            batched_data.append(data[batch])
            batched_label.append(label[batch])
            if i == l-1:
                break
            batch = []
    #############################################################################
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    #############################################################################
    epochs = np.arange(len(train_acc_history))
    epochs += 1

    # training
    l_fig = plt.figure()
    l_ax = plt.axes()
    l_ax.set(title="Loss Curve", xlabel="Epochs", ylabel="Loss")
    l_ax.plot(epochs, train_loss_history, label = 'training loss')
    l_ax.plot(epochs, valid_loss_history, label = 'validation loss')
    l_ax.legend()

    # validation
    a_fig = plt.figure()
    a_ax = plt.axes()
    a_ax.set(title="Accuracy Curve", xlabel="Epochs", ylabel="Accuracy")
    a_ax.plot(epochs, train_acc_history, label = 'training accuracy')
    a_ax.plot(epochs, valid_acc_history, label = 'validation accuracy')
    a_ax.legend()
